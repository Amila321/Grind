...
import { useAuth, type UserDto } from "../hooks/useAuth";

type FriendProfileLocationState = {
    username?: string;
};
...
export function FriendProfilePage() {
    ...
    const [friendUser, setFriendUser] = useState<UserDto | null>(null);
    ...
    const locationState = location.state as FriendProfileLocationState | null;

    const friendUsername = useMemo(() => {
        return friendUser?.username ?? locationState?.username ?? `User ${parsedUserId}`;
    }, [friendUser?.username, locationState?.username, parsedUserId]);
    ...
    const loadFriendProfileStats = useCallback(async () => {
        ...
        try {
            const [
                ...
                getCompletedDaysCountByUserId(parsedUserId),
                getCompletedDaysByUserId(parsedUserId),
            ]);

            setFriendUser(userResponse);
    ...
    useEffect(() => {
        ...
        // Schedule the call asynchronously to avoid setState in effect warning
        queueMicrotask(() => {
            loadFriendProfileStats();
        });
    }, [navigate, loadFriendProfileStats]);
            ...
            <main className="min-h-screen bg-background p-8">
                <div className="flex min-h-[60vh] items-center justify-center">
                    <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span>Loading friend profile...</span>
                    </div>
                </div>
            </main>
        );
                ...
                <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
                    <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm text-muted-foreground">Friend Profile</p>
